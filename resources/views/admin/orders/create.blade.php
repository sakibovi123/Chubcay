...
<div class="w-full my-5 p-3 container md:mx-auto">
    ...
    <form class="w-full my-7" action="{{ route('checkout.store') }}" method="post">
        @csrf
            ...
            <div class="w-full">
                <label class="text-gray-600 font-bold text-md">Select Payment Option</label><br>
                
                <select id="payment_method" required name="payment_option" class="p-2 rounded w-full border">
            ...
            <div id="cardOrcash" class="w-full hidden">
        ...
        <div id="customFieldSection" class="hidden flex flex-col items-center gap-3 my-2">
            <div id="customFields" class="w-full flex items-center justify-between gap-3 mt-2">
                <input name="custom_fields[0][key]" placeholder="Enter key" class="w-full p-2 rounded border" type="text">
                <input name="custom_fields[0][value]" placeholder="Enter value" class="w-full p-2 rounded border" type="text">
            ...
            $(document).ready(function() {
                $('#payment_method').on('change', function() {
                    if ($(this).val() === 'pay_now') {
                        ...
                        $('#term').show();  // Show term section
                        $('#paymentTerm').attr('required', true);  // Make paymentTerm required
                    } else {
                        $('#method').removeAttr('required');
                        $('#cardOrcash').hide();  // Hide cardOrcash section
                        $('#term').hide();  // Hide term section
                        $('#paymentTerm').removeAttr('required');  // Remove required from paymentTerm
